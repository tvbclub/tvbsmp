...
public class RankManager {
    ...
    public void loadConfig(FileConfiguration cfg) {
        ranks.clear();
        if (cfg == null || !cfg.isConfigurationSection("ranks")) return;
        for (String key : cfg.getConfigurationSection("ranks").getKeys(false)) {
            ...
            else map.put("requirements", Collections.emptyMap());
    ...
    public boolean canRankUp(Player p, PlayerData data, List<String> failReasons) {
        ...
        if (!nextOpt.isPresent()) { failReasons.add("You are at the highest rank."); return false; }
        ...
        for (RankRequirement r : requirements) {
            if (!r.matches(p, data, reqs, this)) { failReasons.add(r.getFailureMessage(reqs)); return false; }
        }
        return true;
    }
